...
| Clipboard | expo-clipboard | `~56.0.4` |
| Seletor de arquivo | expo-document-picker | `~56.0.4` |
...
│   │   ├── ListingScreen.jsx
│   │   ├── CheckoutScreen.jsx
│   │   ├── PixPaymentScreen.jsx           # PIX manual: mostra chave/QR fixos da loja (configuracoes/pagamento) para o cliente pagar; botão "enviar comprovante" (navega para UploadComprovante) e "Pagar depois" (MyOrders)
│   │   ├── UploadComprovanteScreen.jsx    # último passo do PIX manual: seleciona foto/PDF do comprovante (expo-document-picker), sobe pro Cloudinary e marca o pedido como 'Aguardando confirmação'
...
│   │   ├── cloudinary.js               # uploadComprovante(fileUri, fileName, mimeType) — upload de comprovante PIX (imagem/PDF) para o Cloudinary, public_id aleatório
...
│   │                                   # getOrder, addOrder, marcarComprovanteEnviado
│   └── theme/
...
✅ **Remoção de console.log com dados sensíveis (2026-07-20)** — removidos logs de debug que expunham QR code PIX, UID de usuários e respostas completas de pagamento no console do navegador: `CheckoutScreen.jsx` (fetch de frete com body/status/data completos; resposta completa do PIX incluindo QR code; uid/orderId ao salvar PIX no Firestore; resposta completa do pagamento com cartão), `OrderTrackingScreen.jsx` (pedido inteiro incluindo QR code e status PIX) e `firestore.js` (`savePixData`: orderId e uid). Mantidos intactos todos os `console.warn`/`console.error` de tratamento de erro já existentes — apenas logs de debug informativo foram removidos.
✅ **CheckoutScreen.jsx: feedback visível em falha de confirmação de pedido (2026-07-20)** — anteriormente, se `addOrder` ou outra etapa não tratada falhasse no `handleConfirm`, o catch mais externo só fazia `console.warn('[Checkout] error', e)` — o botão parava de carregar silenciosamente, sem nenhuma explicação ao usuário. O estado `checkoutError` e o banner de erro no JSX (com ícone, botão de fechar e cor `#c0392b`, junto ao botão "Confirmar Pagamento") já existiam de sessão anterior (usados para validações de endereço/frete/PIX pendente); reaproveitados em vez de criar um padrão novo. Mudanças: (1) catch mais externo agora também chama `setCheckoutError('Não foi possível confirmar seu pedido. Verifique sua conexão e tente novamente.')`; `setConfirming(false)` permanece apenas no `finally` já existente, sem duplicação; (2) `setCheckoutError('')` adicionado logo antes de `setConfirming(true)`, no início do fluxo de confirmação, para não exibir uma mensagem de erro antiga numa nova tentativa.
✅ **CheckoutScreen.jsx: limpeza do polling do PIX ao sair da tela + limite de tentativas (2026-07-20)** — o `setInterval` de verificação de status do PIX (a cada 5s, dentro de `gerarPixReal`) era guardado em `pollingInterval`, mas nada o lia nem havia cleanup no desmonte do componente; se o usuário saísse da tela de Checkout com um PIX pendente (voltar, trocar de aba, fechar o app), o polling continuava rodando indefinidamente em segundo plano, podendo disparar `navigation.navigate` bem mais tarde de forma inesperada. Adicionado `useEffect` de cleanup (`return () => { if (pollingInterval) clearInterval(pollingInterval); }`, dependência `[pollingInterval]`) próximo aos demais `useEffect` do componente. Também adicionado limite de tentativas dentro do próprio callback do `setInterval`: contador local `tentativas` incrementado a cada execução, `MAX_TENTATIVAS = 60` (60 × 5s = 5 minutos); ao exceder, chama `clearInterval(interval)` e loga aviso, encerrando o polling automaticamente mesmo se o usuário permanecer na tela sem pagar. Nenhum nome de função/variável existente foi alterado.
✅ **firestore.js: getProductById otimizado (2026-07-20)** — anteriormente lia todo o catálogo (`fetchAll()`, que busca e mapeia todos os documentos de `/produtos`) só para filtrar um único item via `.find()`; era chamado uma vez por item do carrinho (em paralelo, via `items.map`) dentro de `calculateShipping()` em `CheckoutScreen.jsx` — no momento mais crítico do fluxo, o cálculo de frete no checkout. Agora busca diretamente o documento com `getDoc(doc(db, COL, String(id)))`, reduzindo drasticamente as leituras do Firestore nesse ponto. Para manter compatibilidade exata com o formato que `fetchAll()` já produzia por item, o snapshot passa pela mesma função `mapProduct()` usada internamente por `fetchAll()` (aceita tanto `QueryDocumentSnapshot` quanto `DocumentSnapshot`, já que ambos expõem `.id` e `.data()`) — preserva os campos calculados/normalizados (`sale`, `rating`, `reviewCount`, `weights`, `colors`, `categoryLabel`, `weight*`) e o filtro de `visible !== false` / `status !== 'Inativo'` (retorna `null` para produto oculto/inativo, igual ao comportamento anterior). Assinatura `getProductById(id)` inalterada; `doc`/`getDoc` já estavam importados de `firebase/firestore`. Único chamador real no projeto é `CheckoutScreen.jsx` (`calculateShipping`, via `items.map(item => getProductById(String(item.id)))`) — não precisou de nenhum ajuste, pois já usa optional chaining com fallback (`product?.weight || 0.3` etc.) e os campos lidos (`weight`, `weightHeight`, `weightWidth`, `weightLength`) são exatamente os que `mapProduct()` normaliza.
✅ **PIX migrado de Mercado Pago para PagBank (2026-07-27)** — `api/criar-pagamento-pix.js` reescrito: chama `POST https://api.pagbank.com.br/charges` (payload `reference_id`/`amount.value` em centavos/`payment_method.type: 'pix'`/`customer.email`) usando `process.env.PAGBANK_TOKEN` em vez de `MP_ACCESS_TOKEN`; mantém a autenticação Firebase (`auth.verifyIdToken`) e a validação de total contra `users/{uid}/orders/{orderId}` já existentes. **Contrato de resposta para o app propositalmente preservado** (`{ id, status, qr_code, qr_code_base64 }`) para não exigir mudanças em `CheckoutScreen.jsx`, que já lê exatamente esses 4 campos (`data.qr_code_base64` é renderizado direto como `data:image/png;base64,...`); como o QR Code do PagBank vem como texto copia-e-cola (`qr_codes[0].text`/`.emv`) + link de imagem (`qr_codes[0].links[].href`, não como base64 pronto), o endpoint agora baixa a imagem do link retornado e converte para base64 no servidor antes de responder. `api/verificar-pagamento.js` também migrado (estava fora do pedido original, mas ficaria quebrado se não migrado junto — o polling do app chama esse endpoint com o `id` retornado pela criação do PIX): agora consulta `GET https://api.pagbank.com.br/charges/{id}` e traduz o vocabulário de status do PagBank (`PAID`/`DECLINED`/`CANCELED`/`WAITING`/`IN_ANALYSIS`/`AUTHORIZED`) para os valores que `CheckoutScreen.jsx` já compara literalmente (`approved`/`rejected`/`cancelled`/`pending`) via `STATUS_MAP`. `firestore.js` (`savePixData`/`updatePixStatus`) não precisou de alteração — já usa nomes de campo genéricos (`pixId`, `pixQrCode`, `pixQrCodeBase64`, `pixStatus`). **Cartão de crédito permanece no Mercado Pago** (`api/criar-pagamento-cartao.js` não foi tocado — fora do escopo pedido). Requer nova env var `PAGBANK_TOKEN` no Vercel (substitui `MP_ACCESS_TOKEN` apenas para o fluxo PIX); `MP_ACCESS_TOKEN` continua necessário para cartão. **Não testado contra a API real do PagBank nesta sessão** (sem credenciais sandbox disponíveis) — os nomes exatos de campo (`qr_codes[0].text` vs `.emv`, `links[].rel`) foram inferidos da documentação pública do PagBank e devem ser validados no primeiro teste real antes de considerar o fluxo pronto para produção. Alterações enviadas para `origin/main` em 2026-07-27 (commit `0e33322`) — deploy automático via Vercel acionado.
✅ **Fix: trim defensivo no PAGBANK_TOKEN (2026-07-27, commit `ac6db34`)** — logs de produção reportaram `TypeError: Headers.append: 'Bearer a500bfbd-...'` ao chamar a API do PagBank. Investigação confirmou que `criar-pagamento-pix.js` **já usava um objeto simples** como `headers` do `fetch` (`{ Authorization: ..., 'Content-Type': ... }`) — não havia `Headers.append()` em nenhum lugar do arquivo nem de `api/`. A causa raiz não estava no código, e sim em caractere inválido (espaço/quebra de linha) no valor da env var `PAGBANK_TOKEN` no Vercel; usuário conferiu e corrigiu a env var diretamente no painel. Adicionado `.trim()` defensivo em `PAGBANK_TOKEN` como proteção adicional contra esse tipo de problema no futuro. Enviado para `origin/main`.
✅ **Reescrita de `criar-pagamento-pix.js` no estilo `const handler = async (...) => {}` (2026-07-27)** — usuário pediu reescrita completa "do zero" com um template de referência. O template original recebido removia a checagem de token Firebase (`auth.verifyIdToken`), a validação de que o valor cobrado bate com `users/{uid}/orders/{orderId}` (proteção contra manipular o valor do PIX), os headers de CORS, e usava nomes de campo (`pedido_id`, `valor_total`, `email_cliente`) incompatíveis com o que `CheckoutScreen.jsx` de fato envia (`total`, `email`, `orderId`, `description`) — colar o template literal quebraria o checkout e deixaria o endpoint sem autenticação. Confirmado com o usuário: reescrito no estilo pedido (`const handler = async (req, res) => {...}; export default handler;`, mensagens de erro genéricas `'Falha ao criar pagamento'`/`'Erro interno'`) **mantendo** auth Firebase, validação de total e o contrato de resposta já consumido pelo app (`{ id, status, qr_code, qr_code_base64 }`, com `qr_code_base64` baixado e convertido a partir do link de imagem do PagBank, como antes). Nenhuma mudança de comportamento real — refatoração de estilo sobre a lógica já existente.
✅ **Logging detalhado em `criar-pagamento-pix.js` para debug de fetch failure (2026-07-27, commit `5f86864`)** — adicionados logs de: presença/tamanho do `PAGBANK_TOKEN` (log mostra `length` + 4 primeiros chars, não o token completo); `orderId`/`total`/presença de e-mail recebidos; URL e body da requisição ao PagBank (com e-mail do cliente omitido no log do body); `response.status`/`statusText`; `try/catch` dedicado em volta do `fetch` logando `error.message`/`error.name`/`error.cause` para diferenciar falha de rede de erro HTTP; `chargeId`/`status`/presença de EMV/QR URL retornados. **Divergência deliberada do pedido original:** não loga os 20 primeiros chars do token (log de tamanho+4 chars é suficiente para confirmar presença/formato sem reduzir o espaço de busca de quem tiver acesso aos logs do Vercel) nem o `JSON.stringify(data)` completo da resposta do PagBank (contém o QR code copia-e-cola do PIX — mesma categoria de dado sensível já removida de logs em 2026-07-20); no caso de resposta sem QR code, loga `Object.keys(data)` em vez do conteúdo, preservando a capacidade de diagnóstico sem expor o valor do PIX.
✅ **PIX corrigido para sandbox do PagBank (2026-07-30)** — URL alterada de https://api.pagbank.com.br para https://sandbox.api.pagseguro.com. Token do Vercel precisa ser token SANDBOX (não sk_live). Quando homologação for aprovada, trocar URL e token para produção.
✅ **Mototaxi adicionado como forma de envio (2026-07-30)** — terceira opção de entrega por R$ 8,00, disponível apenas em Itaú de Minas e Passos, prazo até 1 dia útil. Compatível com PIX e Cartão. Clientes em outras cidades veem só Correios com PAC selecionado automaticamente.
✅ **Adicionado productId em cada item de products[] no pedido (addPedidoAdmin) (2026-08-06)** — para permitir baixa de estoque confiável por doc ID no painel admin (fluxo PIX manual). `firestore.js`, `addPedidoAdmin`: cada objeto de `products[]` ganhou o campo `productId: item.id || ''` (`item.id` é o ID do documento do produto no Firestore, já presente em cada item do carrinho via `mapProduct`); demais campos (`n`, `sku`, `p`, `q`, `producer`, `tint`, `initials`) mantidos inalterados. Array `items[]` salvo em `users/{uid}/orders` (via `addOrder`) não precisou de alteração — já é gravado bruto do `CartContext` e cada item já tem `id`. Mudança isolada, sem tocar em `CheckoutScreen.jsx` nem nos endpoints `/api` de pagamento — parte 1 da migração para PIX manual com confirmação pelo admin.
✅ **CheckoutScreen migrado para PIX manual (2026-08-06)** — removido PagBank (`criar-pagamento-pix`), polling (`verificar-pagamento`), QR dinâmico e baixa de estoque no app no caminho PIX. Pedido PIX agora é criado com status `'Aguardando pagamento'` e navega para `PixPayment`. Cartão e boleto intactos. Detalhes: removidas de `CheckoutScreen.jsx` a função `gerarPixReal()` (chamada a `POST /api/criar-pagamento-pix`), a função `copiarPix()`, o `setInterval` de polling (`GET /api/verificar-pagamento`, máx. 60 tentativas x 5s), a chamada a `decrementarEstoque(items)` no caminho PIX (baixa de estoque passa a ser feita pelo painel admin ao confirmar o pagamento manualmente — `decrementarEstoque` continua importada e usada no fluxo de cartão), os estados `pixData`/`pixLoading`/`pixCopied`/`pixGenerated`/`currentOrderId`/`paymentStatus`(PIX)/`pollingInterval`, o `useEffect` de cleanup do polling, o contador `seconds`/`mmss` (countdown de 15 min do QR inline) e as constantes `PIX_API_URL`/`VERIFICAR_API_URL`; import de `savePixData`/`updatePixStatus` removido de `firestore.js` (funções permanecem no arquivo, só não são mais chamadas por `CheckoutScreen`). O bloco JSX da aba PIX (QR Code, código copia-e-cola, countdown, estados aprovado/rejeitado) foi substituído por um texto estático explicando que a chave/QR da loja serão exibidos após confirmar. `handleConfirm`: removido o bloqueio de "PIX já gerado mas não pago"; branch `tab === 'pix'` agora só chama `clearCart()` e `navigation.navigate('PixPayment', { orderId, total: checkoutTotal })` logo após `addOrder`+`addPedidoAdmin` (que continuam gravando `status: 'Aguardando pagamento'` para PIX, como já faziam). Texto do botão de confirmar simplificado — não depende mais de `pixGenerated`/`paymentStatus`. Fluxo de cartão (tokenização MP, `criar-pagamento-cartao`, `decrementarEstoque` pós-aprovação) e boleto (navegação direta com `paymentStatus: 'pending'`) **não foram alterados**. Estilos `pixWrap`/`qrCode`/`pixCopy`/`pixCode`/`copyBtn`/`countdownRow`/`countdownText` ficaram sem uso em `CheckoutScreen.jsx` (não removidos do `StyleSheet` — fora do escopo desta mudança, limpeza pode ser feita depois).
✅ **Criada PixPaymentScreen (2026-08-06)** — mostra QR + chave PIX fixos da loja lidos de `configuracoes/pagamento` via `getConfigPagamento()` e registrada a rota no `AppNavigator`. Fluxo PIX manual: cliente paga e segue para envio de comprovante. Detalhes: `getConfigPagamento()` adicionada em `firestore.js` — lê `doc(db, 'configuracoes', 'pagamento')` diretamente (distinta de `getConfiguracoes()`, que agrega toda a coleção `/configuracoes` numa única resposta); retorna o objeto do doc (`{ pixChave, pixQrCodeUrl, pixAtivo, ... }`) ou `null` se não existir. `PixPaymentScreen.jsx` criada: recebe `{ orderId, total }` via `route.params`; ao montar, busca a config com loading (`ActivityIndicator`); exibe o total em destaque (`fmt()`), a imagem do QR (`Image` com `pixQrCodeUrl`, com aviso "QR Code indisponível, use a chave abaixo" quando ausente), a chave PIX num box com botão "Copiar chave"/"Copiada!", e um passo a passo de 4 itens em PT-BR; botão primário "Já paguei — enviar comprovante" navega para `navigation.navigate('UploadComprovante', { orderId, total })` (tela ainda não criada — só a navegação); botão secundário "Pagar depois" navega para `MyOrders`. Na época desta entrada, `expo-clipboard` ainda não estava instalado e a cópia usava `navigator.clipboard` (web-only) — ver correção abaixo. `AppNavigator.jsx`: import de `PixPaymentScreen` e `<Stack.Screen name="PixPayment" component={PixPaymentScreen} />` adicionados logo após `Checkout` — resolve o erro de navegação que existia desde a migração do `CheckoutScreen` para PIX manual.
✅ **Instalado expo-clipboard e substituído navigator.clipboard (2026-08-06)** — web-only na `PixPaymentScreen` (cópia da chave PIX) e na `OrderTrackingScreen` (código de rastreio) — cópia agora funciona em web, iOS e Android. `expo-clipboard@~56.0.4` instalado via `npx expo install expo-clipboard` (versão compatível com SDK 56). `PixPaymentScreen.jsx`: `import * as Clipboard from 'expo-clipboard'` adicionado; `handleCopy()` trocado de `if (Platform.OS === 'web' && navigator?.clipboard) { await navigator.clipboard.writeText(...) }` para `await Clipboard.setStringAsync(config.pixChave)`; import de `Platform` removido do arquivo (não era mais usado em nenhum outro ponto). `OrderTrackingScreen.jsx`: mesma troca em `handleCopy(text)` — `await Clipboard.setStringAsync(text)`; import de `Platform` removido pelo mesmo motivo. Feedback visual "Copiada!"/`copied` mantido intacto nos dois arquivos. Nenhum outro trecho dos dois arquivos foi alterado.
✅ **Criada UploadComprovanteScreen (2026-08-06)** — upload de comprovante foto/PDF para Cloudinary com public_id aleatório + `marcarComprovanteEnviado` (status 'Aguardando confirmação' nas duas coleções) + rota registrada + OrderConfirmation trata o novo estado. Fluxo PIX manual do lado do cliente completo. Detalhes: `expo-document-picker@~56.0.4` instalado via `npx expo install` (compatível com SDK 56). `src/services/cloudinary.js` criado: `uploadComprovante(fileUri, fileName, mimeType)` monta um `FormData` (aceita tanto `{ uri, name, type }` nativo quanto `File`/`Blob` da web — detecta por `instanceof Blob`) e envia para `https://api.cloudinary.com/v1_1/dv62fwdtv/{resourceType}/upload` (cloud name `dv62fwdtv`, upload preset unsigned `emporio-produtos`); `resourceType` é `raw` para PDF, `image` para imagem; `folder: 'emporio-minas/comprovantes'`; `public_id` gerado aleatoriamente (`comprovante_<timestamp base36>_<random>`) por ser dado sensível; erros de rede e de resposta não-OK lançam mensagem amigável. `marcarComprovanteEnviado(uid, orderId, comprovanteUrl)` adicionada em `firestore.js`: grava `{ status: 'Aguardando confirmação', comprovanteUrl, comprovanteEnviadoEm: serverTimestamp() }` com `merge: true` em `users/{uid}/orders/{orderId}` **e** em `pedidos/{orderId}` (mesmo padrão de espelhamento das demais funções de pedido). `UploadComprovanteScreen.jsx` criada: recebe `{ orderId, total }` via `route.params`; botão "Escolher arquivo" abre `expo-document-picker` (`type: ['image/*', 'application/pdf']`, `copyToCacheDirectory: true`); preview de imagem via `<Image>` ou ícone genérico para PDF; botão primário "Enviar comprovante" chama `uploadComprovante()` (usa `file.webFile` quando presente — preenchido pelo picker na web — senão `file.uri` nativo) seguido de `marcarComprovanteEnviado()`, depois navega para `OrderConfirmation` com `{ orderId, paymentStatus: 'awaiting-confirmation' }`; desabilitado durante o envio; erros exibidos inline sem travar a tela (permite tentar de novo); link "Enviar depois" navega para `MyOrders` (pedido continua `'Aguardando pagamento'` até o comprovante ser enviado). `AppNavigator.jsx`: import + `<Stack.Screen name="UploadComprovante" component={UploadComprovanteScreen} />` registrados logo após `PixPayment` — resolve a navegação pendente desde a criação da `PixPaymentScreen`. `OrderConfirmationScreen.jsx`: `successSub` e o badge de status do card de pagamento ganharam um terceiro branch para `paymentStatus === 'awaiting-confirmation'` ("Comprovante enviado! Estamos conferindo seu pagamento..." + badge laranja "Em conferência"), sem alterar o comportamento dos estados `approved`/`pending` já existentes. Fluxo completo agora navega: `Checkout(PIX)` → `PixPayment` → `UploadComprovante` → `OrderConfirmation`. **Pendente (fora do escopo desta tarefa):** baixa de estoque e confirmação final do pagamento continuam a cargo do painel admin ao revisar `comprovanteUrl` e mudar o status de `'Aguardando confirmação'` para `'Pago'`.
✅ **Atualizado CEP de origem da cotação de frete (37900900 → 37902308) e endereço/Maps da loja no CheckoutScreen para a loja do cliente (2026-08-06)** — Mototáxi (Itaú de Minas/Passos) mantido.
✅ **OrderConfirmationScreen: Resumo do Pedido agora exibe a foto real do produto (images[0] || imageUrl) com fallback para o gradiente, seguindo o padrão de CartScreen/FavoritesScreen. A imagem já estava salva em users/{uid}/orders (2026-08-06)** — `Image` adicionado ao import de `react-native`; `<LinearGradient colors={colors} style={styles.itemImg} />` isolado trocado por `<LinearGradient ... style={styles.itemImg}>` como wrapper contendo `<Image source={{ uri: (it.images && it.images[0]) || it.imageUrl }} style={StyleSheet.absoluteFillObject} resizeMode="cover" />` condicional, idêntico ao padrão já usado em `CartScreen.jsx`/`FavoritesScreen.jsx`; sem imagem, cai no gradiente (`colors` do item ou fallback `['#e0c090', '#a07030']`) como antes.

---

## 11. O Que Ainda Falta

❌ **`configuracoes/pagamento` precisa dos campos `pixChave`/`pixQrCodeUrl`/`pixAtivo` cadastrados** — `PixPaymentScreen` já lê esses campos via `getConfigPagamento()`, mas eles precisam existir no painel admin (cadastro da chave PIX fixa da loja e upload da imagem do QR Code) para a tela funcionar de fato; sem isso, exibe "Chave não configurada" e o aviso de QR indisponível
❌ **Confirmação manual do PIX + baixa de estoque no painel admin** — o app já leva o cliente até `'Aguardando confirmação'` com o `comprovanteUrl` salvo (`UploadComprovanteScreen`); falta o painel admin exibir o comprovante, permitir o admin confirmar o pagamento (mudando o status para `'Pago'`) e disparar `decrementarEstoque` nesse momento — hoje isso não é feito automaticamente por nenhum lado
...
0. **Painel admin: revisar comprovante PIX + confirmar pagamento + baixar estoque** — 🔴 urgente: o app já grava `comprovanteUrl` e `status: 'Aguardando confirmação'` em `users/{uid}/orders/{orderId}` e `pedidos/{orderId}` (via `marcarComprovanteEnviado`); falta o painel admin exibir esse comprovante, dar ao admin um botão para confirmar o pagamento (→ `status: 'Pago'`) e chamar a baixa de estoque nesse momento
