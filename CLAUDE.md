...
✅ **ProductDetailScreen — card de entrega com peso e dimensões reais** (2026-06-22) — texto fixo "Entrega em até 3 dias úteis · Frete a partir de R$ 15,90" substituído por: mensagem dinâmica "Frete calculado no checkout com base no seu CEP" + linha de peso/dimensões (`📦 Peso: Xkg  ·  H×W×L cm`) exibida condicionalmente quando `product.weight > 0`; peso abaixo de 1kg exibido em gramas; dimensões exibidas apenas quando os 3 campos (`weightHeight`, `weightWidth`, `weightLength`) são maiores que zero
