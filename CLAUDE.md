...
✅ **Fix: badge do sino conectado ao count real de notificações não lidas** (2026-06-16) — `HomeScreen` importa `getUnreadCount` do `NotificationsPanel` e `useAuth` do `AuthContext`; `useEffect` busca count ao montar (depende de `user?.uid`); `bellDot` renderizado condicionalmente `{unreadCount > 0 && <View style={styles.bellDot} />}` — ponto laranja some automaticamente quando não há notificações não lidas
