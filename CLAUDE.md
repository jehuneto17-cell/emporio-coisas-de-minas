...
✅ **HomeScreen: seções Doces em Geral e Antepastos, Patês e Pastas substituem Mais Vendidos** (2026-06-21) — seção "Mais Vendidos" removida; dois novos `useState` (`doces`, `antepastos`) e carregamento via `getProductsByCategory('TyLolkWBnAXMLgxCwL75')` e `getProductsByCategory('gAFuanOffULW48wD066v')` adicionados ao `Promise.all` do `useEffect`; cada seção exibe lista horizontal de `ProductCard` com até 6 itens; "Ver todos" navega para `SubcategoryScreen` com o ID da categoria correspondente
